...
import joblib
import warnings
import time  # Import time for potential delays in geocoding
...
# Helper function to get coordinates for cities using geopy, cached for performance
@st.cache_data
def get_city_coordinates(cities):
    geolocator = Nominatim(user_agent="india_air_quality_app_v1")
    city_coords_map = {}
    for city_name in cities:
        try:
            location = geolocator.geocode(f"{city_name}, India", timeout=10) # Added timeout
            if location:
                city_coords_map[city_name] = (location.latitude, location.longitude)
            else:
                st.warning(f"Could not find coordinates for {city_name}")
        except Exception as e:
            st.warning(f"Error geocoding {city_name}: {e}")
            time.sleep(1) # Add a small delay to avoid hitting rate limits if many fail
...
with tab6:
    ...
    city_aqi_data = df.groupby('City')['AQI'].mean().round(0).reset_index()

    # Get unique cities for geocoding
    unique_cities = city_aqi_data['City'].unique()

    # Get coordinates for all unique cities using the cached function
    coordinates_map = get_city_coordinates(unique_cities)

    # Add Latitude and Longitude columns to city_aqi_data
    city_aqi_data['lat'] = city_aqi_data['City'].map(lambda x: coordinates_map.get(x, (None, None))[0])
    city_aqi_data['lon'] = city_aqi_data['City'].map(lambda x: coordinates_map.get(x, (None, None))[1])

    # Filter out cities for which coordinates could not be found
    city_aqi_data = city_aqi_data.dropna(subset=['lat', 'lon'])

    # Create a Scatter Mapbox plot if there's data to display
    if not city_aqi_data.empty:
        ...
        st.warning("No city coordinates found to display on the map.")
